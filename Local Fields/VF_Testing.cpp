...
void VectorFields::testProjection_MyBasis_NoRegularizer(const Eigen::SparseMatrix<double>& U, const Eigen::PardisoLDLT<Eigen::SparseMatrix<double>> &sparseSolver, const Eigen::SparseMatrix<double>& B, const Eigen::VectorXd& a, const Eigen::VectorXd& v, double &error)
{
	...
	if (writeToFile) {
		std::ofstream ofs;
		string resultFile = "D:/Nasikun/4_SCHOOL/TU Delft/Research/Projects/LocalFields/Data/Tests/Projections/Fertility_L2projection_eigenFields_" + to_string(Basis.cols()) + "_80sup.txt";
...
void VectorFields::testProjection_MyBasis_WithRegularizer(const Eigen::SparseMatrix<double>& Basis, const Eigen::VectorXd& inputFields, const Eigen::SparseMatrix<double>& MReg, double &error)
{
	...
	if (writeToFile) {
		...
		string resultFile = "D:/Nasikun/4_SCHOOL/TU Delft/Research/Projects/LocalFields/Data/Tests/Projections/Fertility_L2projectionWithReg_eigenFields_" + to_string(Basis.cols()) + "_80sup.txt";
...
void VectorFields::projectionTest(bool &readDesFieldsFromFile, bool &readPertFieldsFromFile, bool &useEigenBasis, int start, int nTests)
{
	...
	if (useEigenBasis)
	{
		...
		ReadDenseMatrixFromMatlab(EigenBasisFull, eigBasisFile, 172964, 1000);
		// same storage
...
void VectorFields::convergenceTest()
{
	...
	for (int i : subspdim) {
		basisFile.push_back("D:/Nasikun/4_SCHOOL/TU Delft/Research/Projects/LocalFields/Data/Basis/Basis_Fertility_" + to_string(i) + "_Eigfields_80sup");
	}
