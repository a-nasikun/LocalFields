...
//>>>>>>> master
	...
	std::cout << "Computing the rotation angle \n";
	...
	std::cout << "Defining the rotation angle \n";
	...
	std::cout << "Selecting face to draw \n";
...
{	
	...
	//const double EDGE_RATIO = 10;				// for patch of the eigenfields (cube)
...
void VectorFields::visualize3Dfields(igl::opengl::glfw::Viewer &viewer, const Eigen::VectorXd &field3D, const Eigen::RowVector3d &color)
{
	...
	/* converting vector fields as matrix */
	Eigen::MatrixXd VectorRow(F.rows(), 3);
	VectorRow.setZero();
	for (int i = 0; i < F.rows(); ++i) {
		VectorRow.row(i) = field3D.block(3 * i, 0, 3, 1).transpose();
	}

	//averageGrad = totalGrad / (double)F.rows();
	double lengthScale = 0.2*avgEdgeLength; // / averageGrad;

	viewer.data().add_edges(FC, FC + lengthScale*VectorRow, color);
...
void VectorFields::visualizeApproxResult(igl::opengl::glfw::Viewer &viewer)
{
	...
	Eigen::RowVector3d color, colorInput;	
	color = Eigen::RowVector3d(0.9, 0.1, 0.1);
	//colorInput = Eigen::RowVector3d(0.96470588235, 0.68235294117, 0.17647058823);
	//colorInput = Eigen::RowVector3d(0.56862745098, 0.81176470588, 0.37647058823);
...
void VectorFields::visualizeSubdomain(igl::opengl::glfw::Viewer &viewer, const int id)
{
	/* Color map => JET */
	...
	vColor.setConstant(0.93333333);
	std::cout << "Visualizaing domain #" << id << std::endl;
	for (int i = 0; i < F.rows(); i++)
	{
		 if (localSystems[id](i) > 0.25)
		{
	...
	int fId = Sample[id];
	Eigen::RowVector3d fCoord = (V.row(F(fId, 0)) + V.row(F(fId, 1)) + V.row(F(fId, 2))) / 3.0;
	viewer.data().point_size = 15.0;
	//viewer.data().add_points(fCoord, Eigen::RowVector3d(0.9, 0.1, 0.2));
